...
while not exit: 
    ...
    while True:
        ...
        try: 
            ...
            if 'Check if your spelling is correct, or try removing filters.' not in extra_page:
                ...
                print(f'\nFound the last page, total pages of data aquired: {page}')
                break
    ...
    if download_all == True:
        try:
            os.mkdir(f'./FOIA_downloads_{search_term}')
        except:
            print(f'\ndownload folder for {search_term} already exists, adding files to that folder.')
        # this will keep us from downloading a bunch from the CIA's website without need. 
        for i, link in enumerate(search_links):
        # for each link in our search link list, goto the subject page, find the pdf, save it to a file. * 
            ...
            for pdf_url in pdf_list: # this loops through the list of URLs and downlaods the pdfs. 
                ...
                try:
                    with open(f'./FOIA_downloads_{search_term}/FOI_{search_term}_{pdf_url[-14:-4]}_.pdf', 'wb') as new_file:
                        # this creates a new file, under FOI_searchterm_doc#.pdf
                       new_file.write(pdf_data.content)
                except: 
                    # this will assist in any errors
                    print(f'\n{"*"*50}\nThere was an error downloading file #{i+1}')
